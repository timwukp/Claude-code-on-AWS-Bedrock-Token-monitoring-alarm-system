<h1 align="center">Token Usage Monitoring System</h1>

<p align="center">
  <b>Know what your Amazon Bedrock spend was, who caused it, and whether it was worth it.</b><br>
  A cross-industry, multi-tenant platform for monitoring token usage, detecting cost anomalies,
  automating incident response, and running forensic analytics — deployable into any AWS account.<br>
  Built AWS-native with the
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/IaC-AWS%20CDK%20(TypeScript)-orange" alt="IaC: AWS CDK (TypeScript)">
  <img src="https://img.shields.io/badge/stacks-10%20independently%20deployable-informational" alt="10 independently deployable stacks">
  <img src="https://img.shields.io/badge/Well--Architected-6%20pillars%20reviewed-success" alt="Well-Architected: 6 pillars reviewed">
</p>

---

## 🎬 Watch the dashboard — 8 minutes, narrated

https://github.com/user-attachments/assets/ebd6b51a-bd40-4e24-91da-c512a6052351

A signed-in walkthrough of all ten pages against a **live deployment**, reading real data from a real AWS account. ([Also on the wiki](https://github.com/timwukp/Claude-code-on-AWS-Bedrock-Token-monitoring-alarm-system/wiki).)

| | Chapter | What you see |
|---:|---|---|
| `0:00` | **The question** | Why Bedrock cost is harder to answer than it looks |
| `0:42` | **Overview** | Spend, budget, anomalies and delivery on one screen |
| `1:23` | **Usage** | Token time series — and why prompt-cache traffic is kept off it |
| `2:01` | **Cost** | Spend by model, and the prompt-cache savings figure |
| `2:38` | **Attribution** | The four-tier ladder, and the `untagged` row it shrank |
| `3:53` | **Guardrails** | Budget Action hard-stop, per-principal containment, both off by default |
| `4:36` | **Anomalies** | An ML-learned baseline instead of a threshold you guessed |
| `5:07` | **Latency** | The measured hops — and the three drawn as question marks |
| `5:39` | **Delivery** | DORA metrics, each hedged in its own copy |
| `6:07` | **ROI** | The DORA model, the −19%…+56% bracket, and what it refuses to compute |
| `7:09` | **Takeaway** | Five questions, one dashboard |
...
- **The problem** — Bedrock bills per token, has no auto-resetting spend cap (its quotas are rate limits), and its invocation logs name the IAM caller but not the team or project. So the bill arrives without an owner.
- **What this does** — measures Bedrock usage, attributes it to a project without asking callers to cooperate, wires AWS-native governance around it, and reads cost against delivery output.
- **How cost is attributed** — a four-tier precedence ladder led by **tagged application inference profiles**: config-routed, zero per-call effort, and IAM-enforceable.
- **What it will not do** — invent numbers. Estimates are labelled estimates, DORA figures are labelled proxies, unmeasured latency hops are drawn dark, and the ROI page lists what it refuses to compute.
- **Enforcement is opt-in** — the hard-stop and per-principal containment ship off by default.
...
- [Features](#features)
- [Project ↔ token attribution](#project--token-attribution-how-a-token-finds-its-project)
- [Architecture at a glance](#architecture-at-a-glance)
- [Documentation](#documentation)
- [Quick start](#quick-start)
- [Status](#status)
- [Agent-driven QA: the PR #30 convergence case](#agent-driven-qa-the-pr-30-convergence-case)
...
<details>
<summary>🔧 <b>Behind the scenes</b> — data plane, aggregation, detection, response</summary>
...
<details>
<summary>📜 <b>One-time historical treatment</b> (2026-09-17, owner-directed)</summary>

Usage that predates the AIP rollout carried no signal at all (99.7% untagged). It was attributed
once, offline, by correlating each hour’s token usage with per-repo commit timestamps (±2h window;
90% of tokens matched) and written through the same pipeline with the precedence ladder intact —
records with real signals were never overridden, unmatched hours remain `untagged`, and a
`SYSTEM#RETRO` marker makes the migration strictly unrepeatable. The raw invocation logs are
immutable: the Athena “Full” view still reports what was true at call time, while the managed
rollups carry the attribution. Method and share table:
`docs/test-reports/feature-13-project-cost-attribution.md`; full design and verification:
...
Everything runs inside a single AWS account / region. Only the **Fargate ETL task runs inside a
VPC** (private subnets), reaching S3 through a VPC gateway endpoint — the serverless API and
ingestion paths intentionally stay outside the VPC.

The system is organized around **one user-facing request path**, supported by **two background
flows** that supply its data and guard its cost.

<details>
<summary>🔀 <b>How the data flows</b> — the request path and the two background flows</summary>
...
**Start here** → [`docs/DEPLOYMENT.md`](./docs/DEPLOYMENT.md) to deploy it,
[`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md) to understand it,
[`AGENTS.md`](./AGENTS.md) if you are an AI coding agent.
...
| [`docs/ROI_METHODOLOGY.md`](./docs/ROI_METHODOLOGY.md) | The ROI model, its DORA provenance, and every term it refuses to compute |
...
**Prerequisites** — an AWS account + credentials (`aws configure`) with permission to deploy the
stacks; Node.js 20+, npm, AWS CDK v2 (`npm i -g aws-cdk`), Docker (for the Fargate image + Lambda
bundling); Amazon Bedrock model access enabled in your target Region.
...
<details>
<summary>🧱 <b>Tech stack and repository layout</b></summary>
