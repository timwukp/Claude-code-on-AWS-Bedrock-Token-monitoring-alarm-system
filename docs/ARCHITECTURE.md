...
| `GET /v1/latency?window=1\|7\|30` | model-hop latency: p50/p95/p99 end-to-end + time to first token, fleet and per model, plus the hop-observability model | CloudWatch `AWS/Bedrock` (`InvocationLatency`, `TimeToFirstToken`) + `ListInferenceProfiles` for model names |
...
Four design consequences worth stating, because each is a limit rather than a feature:

- **No tenant dimension exists on these metrics**, so `LatencyFn` gets no table grant of any kind —
  its IAM is `cloudwatch:GetMetricData`, `cloudwatch:ListMetrics` and `bedrock:ListInferenceProfiles`
  on `*`, all read-only and all list/read shaped — and the endpoint is honest that it reports an
  account-level fleet view. Per-project latency is a different source entirely — the invocation logs
  — and is not built.
- **CloudWatch reports profile-routed traffic under the inference-profile id**, not a model id, so the
  by-model table has to resolve that id or it shows the reader an opaque twelve-character string.
  `resolveLabel()` maps it back via `ListInferenceProfiles`, and refuses in the two cases where there
  is no single right answer: a profile fanning out to several models keeps the profile's name and
  claims no model, and a profile that cannot be found keeps the raw id. The list call is best-effort —
  any failure degrades to raw ids, because a labelling aid must not be able to fail a metrics read.
