...
| 13 | **End-to-end latency observability** | 🟡 | Partial by design — `/latency` page + `GET /v1/latency` ship the one hop we can actually measure. `LatencyFn` reads CloudWatch `AWS/Bedrock` `InvocationLatency` and `TimeToFirstToken` with native p50/p95/p99, fleet and per model, and grants no table access because these metrics carry no tenant dimension. The page draws the full chain (IDE/CLI → gateway → Bedrock TTFB → Bedrock streaming → Guardrails) but **only measured hops carry a number**; the three dark hops name the telemetry needed to light them (Claude Code OTel export, LiteLLM Prometheus histograms, Converse `trace.guardrailProcessingLatency`), all of which is customer-side. 🟡 rather than ✅ for two reasons stated on the page: the view is **fleet-wide, not per project** (per-project latency requires threading `amazon-bedrock-invocationMetrics` through the ingestion rollups — count/sum/buckets on three aggregates and four `ADD` writers, plus a guarded backfill), and Athena/Full-view parity needs a Glue DDL change the repo does not own. No productivity claim is made: the research found no surviving evidence linking per-request latency to developer output. See [`research-latency-measurement`](./research-latency-measurement.md) and [`test-reports/feature-18`](./test-reports/feature-18-latency-observability.md). |
